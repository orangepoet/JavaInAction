...
class Solution2Test extends Specification {
    ...
    def '上下翻转二叉树'() {
        expect:
        solution2.upsideDownBinaryTree(root) == ans
        where:
        root                                 | ans
        TreeNode.create([1, 2] as Integer[]) | TreeNode.create([2, null, 1] as Integer[])
    }

    def '从未排序的链表中移除重复元素'() {
        when:
        def node = solution2.deleteDuplicatesUnsorted(ListNode.create([1, 2, 3, 2] as int[]))
        then:
        node.toIntArr() == [1, 3] as int[]
    }
    ...
    def '二叉树最长连续序列'() {
        given:
        def root = TreeNode.create([1, 2, 3] as Integer[])
        when:
    ...
    def '二叉树的序列化与反序列化'() {
        ...
        given:
        def treeNode = TreeNode.create(intArr)
        when:
    ...
    def '区间链表反转'() {
        ...
        ListNode ans = solution2.reverseBetween(head, 2, 4)

        then:
        ans == ListNode.create([1, 4, 3, 2, 5] as int[])
    }

    def '不同的二叉搜索树'() {
        expect:
        solution2.generateTrees(n) == ans
